...
export default function UploadPanel() {
  ...
  const addHostedMedia = () => {
    if (!title.trim() || !filename.trim()) {
      setLastMessage("Title and filename are required.");
      return;
    }
    ...
    setFilename("");
    setDuration("1800");
    setLastMessage(`Added "${mediaItem.title}" using path ${mediaItem.file}`);
  };
    ...
    >
      ...
      <div className="grid gap-3">
        ...
        <div className="text-xs" style={{ color: "var(--text-muted)" }}>
          {lastMessage ||
            "Make the final path work in a browser first, then add it."}
        </div>
      </div>
