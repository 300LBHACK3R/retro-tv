...
export default function UploadPanel() {
  ...
  const addHostedMedia = () => {
    if (!title.trim() || !url.trim()) {
      setLastMessage("Title and URL/path are required.");
      return;
    }
  ...
  const handleLocalBrowse = (files: FileList | null) => {
    ...
    const cleanTitle = stripExtension(file.name);

    setTitle(cleanTitle);
    setUrl(`/media/${file.name}`);

    setLastMessage(
      `Loaded "${file.name}". If you want this exact path to work, move the file into /public/media.`
    );
      ...
      <div className="grid gap-3">
        ...
        >
          ...
          <div className="mt-2 text-xs" style={{ color: "var(--text-muted)" }}>
            This only helps fill in the title and path. For public viewers, the
            real video file must exist inside <code>public/media</code>.
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="/media/channel1/ttv-show-1.mp4"
            className="w-full rounded-lg border px-3 py-2"
            style={{
              background: "var(--panel-alt-bg)",
              borderColor: "var(--border)",
        ...
        <div className="text-xs" style={{ color: "var(--text-muted)" }}>
          {lastMessage ||
            "For tonight, use /public/media files or direct public MP4 URLs only."}
        </div>
      </div>
