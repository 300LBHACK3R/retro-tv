...
function canLikelyPlayVideo(file: File) {
...
export default function UploadPanel() {
  ...
  const processFiles = async (files: FileList | File[]) => {
    ...
    if (addedCount > 0 && skippedCount > 0) {
      setLastMessage(
        `Added ${addedCount} file(s) to CH ${channelId}. Skipped ${skippedCount} unsupported/broken file(s).`
      );
    } else if (addedCount > 0) {
      setLastMessage(`Added ${addedCount} file(s) to CH ${channelId}.`);
    } else {
      setLastMessage("No valid playable video files were added.");
    }
    ...
    <div className="rounded border border-blue-700 bg-[#0a2a4a] p-4">
      ...
      <div className="grid gap-3">
        ...
        <div className="text-xs text-blue-200">
          {isUploading
            ? "Uploading, validating, and storing locally..."
            : lastMessage || "Uploads are validated and persisted locally in your browser."}
        </div>
      </div>
