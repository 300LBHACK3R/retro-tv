...
function validCommercialCategory(value: unknown): AdCategory | undefined {
  if (typeof value !== "string") {
    return undefined;
  }
  ...
  return clean ? (clean as AdCategory) : undefined;
}

function validAdChannelTargets(value: unknown): AdChannelTarget[] {
  if (!Array.isArray(value)) {
    return [];
  }
...
function validAdPlacement(value: unknown): AdPlacement | undefined {
  return VALID_AD_PLACEMENTS.includes(value as AdPlacement)
    ? (value as AdPlacement)
    : undefined;
}

function validAdPlacements(value: unknown): AdPlacement[] {
  if (!Array.isArray(value)) {
    return [];
  }
  ...
  return Array.from(
    new Set(
      value
        ...
        .filter((item): item is AdPlacement => Boolean(item)),
    ),
  );
}

function validAdCategories(value: unknown): AdCategory[] {
  if (!Array.isArray(value)) {
    return [];
  }
  ...
  return Array.from(
    new Set(
      value
        .map(validCommercialCategory)
        .filter((item): item is AdCategory => Boolean(item)),
    ),
  );
}

function sanitizeAdPolicy(value: unknown): ChannelAdPolicy {
  ...
  return {
    ...
    maxAdsPerBreak: clamp(validNumber(policy.maxAdsPerBreak, 4), 1, 10),
    targetBreakSeconds: clamp(
      validNumber(policy.targetBreakSeconds, 120),
      15,
      1800,
    ),
    minSecondsBetweenSameAd: clamp(
      validNumber(policy.minSecondsBetweenSameAd, 0),
      0,
      86400,
    ),
  ...
  return {
    ...
    slotLengthSeconds:
      !isCommercial && slotLengthSeconds && slotLengthSeconds > duration
        ? slotLengthSeconds
        : undefined,
    fillSlotWithCommercials: isCommercial
      ? false
      : validBoolean(value.fillSlotWithCommercials),
    ...
    adCategories: isCommercial
      ? adCategories.length > 0
        ...
        : [category]
      : undefined,
    adPriority: isCommercial
      ? Math.max(0, Math.floor(Number(value.adPriority) || 0))
      : undefined,
    adMaxPlaysPerHour: isCommercial
      ? validPositiveInteger(value.adMaxPlaysPerHour)
      : undefined,
    adMinSecondsBetweenPlays: isCommercial
    ...
    advertiserName: isCommercial
      ? validOptionalString(value.advertiserName)
      : undefined,
    campaignName: isCommercial
      ? validOptionalString(value.campaignName)
      : undefined,
  };
}
...
function sanitizeBranding(
  ...
  return {
    ...
    logoUrl: validOptionalString(branding.logoUrl),
  };
}
...
function sanitizeChannel(value: unknown): Channel | null {
  ...
  return {
    ...
    kidsApproved: value.adultOnly !== true && value.kidsApproved === true,
