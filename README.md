...
Eight character portraits (Fox, Space explorer, Dinosaur, Robot, Girl cat, Boy cat, Jesus and Jesus for Kids) replace the old symbols. The avatar picker uses two rows on phones; choosing a portrait never changes a profile’s Kids setting. The existing Cat profile ID now uses Girl cat, so saved profiles still load after the old cat image was replaced. Their stored IDs remain stable, so existing profiles upgrade without being recreated. Images are served locally through Next image optimization.
...
`public/avatars/` retains the four original Fox, Space explorer, Dinosaur and Robot portraits plus the four portraits uploaded in commit `97f10ad`: `girlcat.png`, `boycat.png`, `jesus.png` and `kidsjesus.png`. The uploads are preserved unchanged. The original set was generated for Tate’s TV with the image-generation tool on September 15, 2026. The shared art direction was a polished, friendly animated-film character portrait with expressive eyes, a teal/violet background, centered head and shoulders, no text, no logos, and no existing franchise character. Individual subjects: orange fox in a teal jacket; brown-skinned space explorer in a cream/purple helmet; teal retro-TV robot; mint dinosaur with purple spikes. The original cat was replaced by the uploaded girl and boy cats. These images are used only as profile portraits, not as programme artwork or Kids suitability labels.
